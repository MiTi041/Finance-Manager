# Budgets-Seite Feinschliff — Implementierungsplan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Budgets-Seite aufpolieren: Budget-Karte im Stil der Sparpläne (Status-Pill + Options-Dropdown, `X / Y`-Betragszeile, Prozent-Zeile), aufgeräumter Header, aufgewerteter Add-Dialog, Bearbeiten-Dialog, Cursor-Pointer.

**Architecture:** Ein-File-Änderung an `frontend/src/pages/budgets/budgets-page.tsx`. Kein Backend, keine neuen Dependencies, kein Test-Runner im Frontend (Verifikation über `tsc --noEmit` + `pnpm build`).

**Tech Stack:** React + Tailwind + shadcn-UI (Button, Card, Dialog, Input, Popover, Progress mit motion).
...
- [ ] **Step 1: Datei komplett ersetzen durch folgenden Inhalt** (`write` überschreibt die Datei):
...
- [ ] **Step 2: Verifikation** — Lauf in `frontend/`:
...
git commit -m "style(budgets): restyle budget cards like savings plans (status pill, options menu, edit dialog)"
```
