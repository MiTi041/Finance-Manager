...
const DOT_TAG_REGEX = /tag\.([a-zA-Z0-9_äöüÄÖÜß.]+)/gi;
...
export function extractDotTags(text: string): string[] {
  if (!text) return [];
  const tags = new Set<string>();
  let match;
...
export function isTypingTag(text: string): boolean {
  ...
  return lastWord.startsWith("tag.") && lastWord.length >= 1;
}
