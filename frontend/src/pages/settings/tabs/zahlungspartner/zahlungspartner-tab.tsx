...
export function ZahlungspartnerTab() {
  ...
  const hook = useSettingsTab<ZahlungspartnerRecord, OwnerFormState>({
    entityName: "Zahlungspartner",
    cacheKey: "zahlungspartner-reference-data",
    loadErrorTitle: "Zahlungspartner konnten nicht geladen werden",
    saveErrorTitle: "Zahlungspartner konnten nicht gespeichert werden",
    deleteErrorTitle: "Zahlungspartner konnte nicht gelöscht werden",
    EMPTY_FORM,
    ...
    async (ownerId: number, file: File) => {
      ...
      try {
        ...
        const response = await fetch(
          `${getApiBaseUrl()}/db/reference-data/zahlungspartner/${ownerId}/logo`,
          {
            method: "POST",
            body: formData,
          },
        ...
        if (hook.editingItem?.id === ownerId) {
          const refreshed = await fetchZahlungspartner(ownerId);
  ...
  const filterOwner = (owner: ZahlungspartnerRecord, query: string) => {
    ...
    <div>
      {hook.error ? (
        ...
        <VirtualizedList
          ...
          emptyStateTitle="Keine Zahlungspartner vorhanden"
          emptyStateText="Lege einen Zahlungspartner an, um IBANs und Logos zuzuordnen."
          ...
          filterItems={[
            ...
            >
              ...
              />
              Zahlungspartner ohne Webseite anzeigen
            </Button>,
            <Button
              type="button"
              variant="ghost"
          ...
          renderItem={(owner) => {
            ...
            const rawLogoSrc = resolveZahlungspartnerLogoSrc(
              owner.logo_url,
              owner.id,
