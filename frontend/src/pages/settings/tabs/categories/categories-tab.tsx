...
export function CategoriesTab() {
  ...
  const categoryTotalCount = useMemo(() => {
    ...
    const addDescendants = (id: number): number => {
      const cached = total.get(id);
      if (cached !== undefined) return cached;
      const cat = hook.items.find((c) => c.id === id);
      let sum = cat?.transaction_count ?? 0;
      for (const child of childrenByParent.get(id) ?? []) {
        sum += addDescendants(child.id);
      }
      total.set(id, sum);
      return sum;
    };
    hook.items.forEach((cat) => addDescendants(cat.id));
    return total;
  }, [hook.items, childrenByParent]);
                    ...
                    <div className="relative">
                      <div className="flex items-center gap-2 relative z-10">
                        ...
                        <Badge variant="outline" className="text-[11px] text-muted-foreground">
                          {categoryTotalCount.get(category.id) ?? category.transaction_count}
                        </Badge>
