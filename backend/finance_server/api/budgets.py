...
@router.get("/db/budgets/hashtag-suggestions")
def get_hashtag_suggestions() -> dict[str, Any]:
    return {"hashtags": list_hashtag_suggestions()}
...
def create_budget_endpoint(request: BudgetCreateRequest) -> dict[str, Any]:
    try:
        return create_budget(
            request.name, request.category_ids, request.amount, request.period, request.hashtags
        )
